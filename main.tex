...
\chapter{Proposed Solution}
    ...
    This chapter presents a cryptographic solution for enhancing the security of Health Internet of Things (HIoT) data. Building upon the decentralized storage framework proposed by \citet{laura2023}, this work introduces a cryptographic layer, which was left as an area for future work in the original paper. The solution uses a hybrid model that combines Multi-Authority Attribute-Based Encryption (MA-ABE) for fine-grained access control with the Advanced Encryption Standard (AES) for payload encryption. This model is designed so that computationally intensive operations are performed on small pieces of data, while larger payloads are handled by symmetric algorithms.
...
\label{chap:experimentalsetup}
    ...
    This chapter describes the experimental setup used to evaluate the proposed cryptographic system for Health Internet of Things (HIoT) data management. It outlines the evaluation strategy, testbed environment, and the tools employed for testing.
...
\chapter{Conclusion}
    ...
    \section{Future Work}
        ...
        Atualmente o sistema só opera sobre texto, sendo necessário a conversão de dados binários para strings que podem ficar muito grandes, aumentando o tamanho do payload HTTP. Isso pode ser ineficiente, especialmente para grandes arquivos binários, como imagens ou vídeos, que são comuns em aplicações de HIoT. A conversão para strings base64, por exemplo, aumenta consideravelmente o tamanho do payload devido à codificação. Uma alternativa seria implementar a criptografia de dados binários diretamente, sem a necessidade de conversão para strings. Para isso a api consumiria a request como um multipart/form-data ao invés de um application/json, e o payload seria enviado como um arquivo binário. A criptografia AES poderia ser aplicada diretamente ao arquivo, e o MA-ABE continuaria a operar sobre o hash do arquivo, mantendo a segurança e a integridade dos dados. A resposta retornaria um application/octet-stream contendo o payload criptografado. Isso reduziria a sobrecarga de tamanho e melhoraria a eficiência do sistema, especialmente para aplicações que lidam com grandes volumes de dados binários.
